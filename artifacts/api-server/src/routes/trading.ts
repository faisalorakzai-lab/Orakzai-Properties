...
/* ─────────────────────────────────────────────────────────────────────────
   GET /trading/candles/:projectId?interval=1m|5m|15m|1h|4h|1d
   Returns OHLC candlesticks aggregated from price_history by timeframe.
   AMM formula applied: every $100 buy/sell changes price by ±1%.
───────────────────────────────────────────────────────────────────────── */
...
router.get("/trading/candles/:projectId", async (req: any, res) => {
  try {
    const projectId = Number(req.params.projectId);
    if (isNaN(projectId)) return res.status(400).json({ error: "Invalid project id" });

    const intervalStr = (req.query.interval as string) ?? "15m";
    const intervalSec = INTERVAL_SECONDS[intervalStr] ?? 900;
    ...
    /* Bucket price_history rows into OHLC candles by interval */
    type Candle = { time: number; open: number; high: number; low: number; close: number; volume: number };
    const buckets = new Map<number, Candle>();

    for (const r of rows) {
      const ts    = (r.timestamp instanceof Date ? r.timestamp : new Date(r.timestamp)).getTime() / 1000;
