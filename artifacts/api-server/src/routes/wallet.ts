...
router.get("/wallet/transactions/:txnId", requireAuth, async (req: any, res) => {
  try {
    const { txnId } = req.params;
    const [txn] = await db
      .select()
      .from(walletTransactionsTable)
      .where(and(eq(walletTransactionsTable.txnId, txnId), eq(walletTransactionsTable.userId, req.userId)))
      .limit(1);
