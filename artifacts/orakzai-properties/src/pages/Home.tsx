                ...
                <div className="w-full relative z-10 my-6" onPointerDown={event => { carouselDragStart.current = event.clientX; event.currentTarget.setPointerCapture?.(event.pointerId); }} onPointerUp={finishSwipe} onPointerCancel={finishSwipe} style={{ touchAction: "pan-y" }}>
                  <div className="w-full flex justify-center items-center" style={{ height: 392, perspective: 1100, overflow: "visible" }}>
                    {MARKET_CAROUSEL_CARDS.map((item, idx) => {
                      ...
                      return <motion.div key={item.background} className="w-[270px] h-[350px] sm:w-[300px] sm:h-[380px] rounded-[44px]" animate={{ x: 0, scale: 1, opacity: 1, rotateY: 0, filter: "blur(0px)" }} transition={{ type: "spring", stiffness: 300, damping: 30, mass: .72 }} onClick={() => { if (carouselDidDrag.current) { carouselDidDrag.current = false; return; } if (!active) setActiveCarousel(idx); }} style={{ position: "relative", margin: "0 auto", zIndex: 10, cursor: "grab", transformStyle: "preserve-3d" }}>
