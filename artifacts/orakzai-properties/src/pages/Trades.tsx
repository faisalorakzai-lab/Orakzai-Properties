...
import { Search, ChevronDown, X, AlertTriangle, ArrowLeft, Star, Bell, TrendingUp, TrendingDown, ExternalLink, Settings, MoreVertical } from "lucide-react";
...
const ASSETS = [
  ...
  { ticker:"OKBOND",  pair:"OKBOND/USDT",  name:"Orakzai Bond",   price:0.8800, change:6.50,  high:0.9400, low:0.8200, vol:312000,  volU:274560,  mc:"$4.4M",  supply:"5M OKBOND", maxSup:"50M OKBOND", ath:"$1.20",  atl:"$0.18",  web:"https://orakzaiproperties.com", wp:"https://orakzaiproperties.com/whitepaper", desc:"Native utility token of Orakzai Properties. OKBOND holders receive reduced trading fees, governance voting rights, and staking rewards.", mf:{l:{b:8400,s:6200,i:2200},m:{b:5100,s:3800,i:1300},sm:{b:2600,s:1900,i:700}} },
];
...
/* ── MARKET LIST ─────────────────────────────────────────────────────────────── */
const MAIN_TABS = ["Favorites","Projects","Building","Commercial","Prediction","News","Data"] as const;
type MainTab = typeof MAIN_TABS[number];

const TAB_ASSETS: Record<MainTab,string[]> = {
  "Favorites":  [],
  "Projects":   ["ASC","DHA9","GBR","CSC","OKBOND"],
  "Building":   ["BTI","ASC","DHA9"],
  "Commercial": ["CSC","GBR","BTI"],
  "Prediction": ["ASC","OKBOND","DHA9","CSC"],
  "News":       ["ASC","DHA9","BTI","GBR","CSC","OKBOND"],
  "Data":       ["ASC","DHA9","BTI","GBR","CSC","OKBOND"],
};

const BASE_PAIRS = ["USDT","OKB","USD1","BTC"] as const;
type BasePair = typeof BASE_PAIRS[number];
const SUB_TABS = ["Spot","Tokenized","Futures"] as const;
...
function MarketList({onSelect}:{onSelect:(a:Asset)=>void}) {
  ...
  const [subTab,setSubTab]   = useState<SubTab>("Spot");
  const [basePair,setBasePair] = useState<BasePair>("USDT");
  ...
  const toggleFav=(ticker:string,e:React.MouseEvent)=>{
    e.stopPropagation();
  ...
  const tabFiltered = mainTab==="Favorites"
    ? ASSETS.filter(a=>favorites.has(a.ticker))
    : ASSETS.filter(a=>TAB_ASSETS[mainTab].includes(a.ticker));

  const filtered = tabFiltered
    .filter(a=>a.ticker.toLowerCase().includes(q.toLowerCase())||a.name.toLowerCase().includes(q.toLowerCase()))
    .sort((a,b)=>{
      if(sortKey==="name")   return sortDir*(a.ticker.localeCompare(b.ticker));
  ...
  return(
    <div style={{minHeight:"100dvh",background:BG,color:FG,paddingBottom:64,fontFamily:"'Plus Jakarta Sans','Inter',sans-serif"}}>

      {/* ── STICKY HEADER ── */}
      <div style={{position:"sticky",top:0,zIndex:20,background:BG}}>

        {/* Search bar — full-width with three-dot menu */}
        <div style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px 8px",paddingTop:"calc(env(safe-area-inset-top,10px) + 10px)"}}>
          <div style={{flex:1,display:"flex",alignItems:"center",gap:8,background:CARD,border:`1px solid ${BORD}`,borderRadius:10,padding:"8px 12px"}}>
            <Search size={14} color={DIM} style={{flexShrink:0}}/>
            <input
              value={q} onChange={e=>setQ(e.target.value)}
              placeholder="Search coin pair, project, or asset..."
              style={{flex:1,background:"transparent",border:"none",outline:"none",color:FG,fontSize:12,minWidth:0}}
            />
            {q&&<button onClick={()=>setQ("")} style={{background:"none",border:"none",cursor:"pointer",padding:0,display:"flex",flexShrink:0}}><X size={11} color={DIM}/></button>}
          </div>
          <button style={{background:"none",border:"none",cursor:"pointer",padding:"4px 2px",display:"flex",alignItems:"center",flexShrink:0}}>
            <MoreVertical size={18} color={DIM}/>
          </button>
        </div>

        {/* Main horizontal category tabs */}
        <div style={{display:"flex",overflowX:"auto",scrollbarWidth:"none",padding:"0 4px",borderBottom:`1px solid ${BORD}`}}>
          {MAIN_TABS.map(tab=>{
            const active=tab===mainTab;
            return(
              <button key={tab} onClick={()=>setMainTab(tab)} style={{
                padding:"8px 13px",fontSize:12,fontWeight:active?700:500,border:"none",
                cursor:"pointer",background:"transparent",whiteSpace:"nowrap",
                color:active?GOLD:DIM,
                borderBottom:active?`2px solid ${GOLD}`:"2px solid transparent",
                flexShrink:0,transition:"color .15s",letterSpacing:"0.01em",
              }}>
                {tab}
              </button>
            );
        ...
        {/* Sub-filter row: Spot | Tokenized | Futures + USDT | OKB | USD1 | BTC */}
        <div style={{display:"flex",alignItems:"center",padding:"6px 12px",gap:0,background:BG,borderBottom:`1px solid ${BORD}`,overflowX:"auto",scrollbarWidth:"none"}}>
          {/* Sub-tabs */}
          {(SUB_TABS as readonly string[]).map((t,i)=>{
            const active=t===subTab;
            return(
              <button key={t} onClick={()=>setSubTab(t as SubTab)} style={{
                padding:"3px 9px",fontSize:11,fontWeight:active?700:400,border:"none",
                borderRadius:4,cursor:"pointer",
                background:active?"rgba(255,255,255,0.06)":"transparent",
                color:active?FG:DIM,transition:"all .15s",
                marginRight:i<SUB_TABS.length-1?2:0,flexShrink:0,
              }}>
                {t}
              </button>
            );
          ...
          {/* Divider */}
          <div style={{width:1,height:16,background:BORD,margin:"0 8px",flexShrink:0}}/>
          {/* Base pair chips */}
          {(BASE_PAIRS as readonly string[]).map(bp=>{
            const active=bp===basePair;
            return(
              <button key={bp} onClick={()=>setBasePair(bp as BasePair)} style={{
                padding:"3px 10px",fontSize:11,fontWeight:active?700:500,
                border:`1px solid ${active?"rgba(243,186,47,0.45)":BORD}`,
                borderRadius:4,cursor:"pointer",marginRight:4,
                background:active?"rgba(243,186,47,0.1)":"transparent",
                color:active?GOLD:DIM,transition:"all .15s",flexShrink:0,
              }}>
                {bp}
              </button>
            );
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 96px 72px",padding:"4px 12px",fontSize:10,background:BG,borderBottom:`1px solid ${BORD}`}}>
          <button onClick={()=>toggleSort("name")} style={{display:"flex",alignItems:"center",gap:3,background:"none",border:"none",cursor:"pointer",color:sortKey==="name"?FG:DIM,fontSize:10,padding:0,fontWeight:sortKey==="name"?700:400,textAlign:"left"}}>
            Name / Vol {sortArrow("name")}
          </button>
          <button onClick={()=>toggleSort("price")} style={{display:"flex",alignItems:"center",justifyContent:"flex-end",gap:3,background:"none",border:"none",cursor:"pointer",color:sortKey==="price"?FG:DIM,fontSize:10,padding:0,fontWeight:sortKey==="price"?700:400}}>
            Last Price {sortArrow("price")}
          </button>
          <button onClick={()=>toggleSort("change")} style={{display:"flex",alignItems:"center",justifyContent:"flex-end",gap:3,background:"none",border:"none",cursor:"pointer",color:sortKey==="change"?FG:DIM,fontSize:10,padding:0,fontWeight:sortKey==="change"?700:400}}>
            24h Chg% {sortArrow("change")}
          </button>
        </div>
      </div>
      ...
      {/* ── PAIR LIST ── */}
      {filtered.length===0&&(
        <div style={{padding:48,textAlign:"center",color:DIM,fontSize:12,lineHeight:1.8}}>
          {mainTab==="Favorites"
            ? <>No favorites yet.<br/><span style={{fontSize:10}}>Tap ★ on any row to add it here.</span></>
      ...
      {filtered.map((a,i)=>{
        ...
        const vol24h=(a.volU/1000000).toFixed(2)+"M";
        const isFav=favorites.has(a.ticker);
        return(
          <motion.div
            key={a.ticker}
            initial={{opacity:0,x:-6}} animate={{opacity:1,x:0}} transition={{delay:i*.035}}
            style={{display:"grid",gridTemplateColumns:"1fr 96px 72px",alignItems:"center",padding:"10px 12px",cursor:"pointer",borderBottom:`1px solid ${BORD}`,gap:6,transition:"background .15s"}}
            onMouseEnter={e=>(e.currentTarget.style.background=CARD)}
            onMouseLeave={e=>(e.currentTarget.style.background="transparent")}
          >
            {/* Left — icon badge + token info */}
            <div style={{display:"flex",alignItems:"center",gap:10}} onClick={()=>onSelect(a)}>
              <div style={{
                width:38,height:38,borderRadius:"50%",
                background:"linear-gradient(135deg,rgba(243,186,47,0.2) 0%,rgba(243,186,47,0.05) 100%)",
                border:`1.5px solid rgba(243,186,47,0.28)`,
                display:"flex",alignItems:"center",justifyContent:"center",
                fontSize:9,fontWeight:800,color:GOLD,flexShrink:0,letterSpacing:"0.04em",
                boxShadow:"0 2px 8px rgba(243,186,47,0.08)",
              }}>
                {a.ticker.slice(0,3)}
              </div>
              <div style={{minWidth:0}}>
                <div style={{fontWeight:700,fontSize:13,color:FG,lineHeight:1.25}}>
                  {a.ticker}
                  <span style={{color:DIM,fontWeight:400,fontSize:11}}>/{basePair}</span>
                </div>
                <div style={{fontSize:10,color:DIM,marginTop:1,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{a.name}</div>
                <div style={{fontSize:9,color:"#4d5561",marginTop:1,fontVariantNumeric:"tabular-nums"}}>{vol24h}</div>
              </div>
            </div>

            {/* Center — last price */}
            <div style={{textAlign:"right"}} onClick={()=>onSelect(a)}>
              <div style={{fontWeight:700,fontSize:13,color:isPos?GREEN:RED,fontVariantNumeric:"tabular-nums",lineHeight:1.25}}>
                {a.price.toFixed(4)}
              </div>
              <div style={{fontSize:9,color:DIM,marginTop:2,fontVariantNumeric:"tabular-nums"}}>
                ≈${(a.price*277.96).toFixed(0)}
              </div>
            </div>

            {/* Right — solid change badge + star */}
            <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:6}}>
              <div
                ...
                style={{
                  ...
                  color:"#fff",textAlign:"center",width:64,cursor:"pointer",
                  lineHeight:1.1,letterSpacing:"0.01em",
                  boxShadow:isPos?"0 2px 8px rgba(14,203,129,0.25)":"0 2px 8px rgba(246,70,93,0.25)",
                }}
              >
                {isPos?"+":""}{a.change.toFixed(2)}%
              </div>
              <button
                onClick={e=>toggleFav(a.ticker,e)}
                style={{background:"none",border:"none",cursor:"pointer",padding:0,display:"flex",lineHeight:1}}
              >
                <Star size={11} fill={isFav?GOLD:"none"} color={isFav?GOLD:"#3a3f48"} strokeWidth={isFav?2:1.5}/>
              </button>
            </div>
          </motion.div>
        );
