      ...
      <main className="mx-auto w-full max-w-2xl overflow-x-clip">
        ...
        <section className="flex flex-col gap-4 px-0 pb-4">
          {filtered.map((post) => {
            ...
            return <article key={post.id} onClick={() => setExpandedPosts((current) => current.includes(post.id) ? current.filter((id) => id !== post.id) : [...current, post.id])} className="mx-3 my-0 min-w-0 cursor-pointer space-y-3 overflow-hidden rounded-2xl border border-[#2b3440] bg-[#151a21] p-4 shadow-sm transition-colors active:bg-[#13171d]">
