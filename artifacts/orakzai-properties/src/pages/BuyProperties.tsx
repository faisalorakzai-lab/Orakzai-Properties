...
function PropertyCard({ item, favorite, onFavorite, onDetails, onChat }: { item: Listing; favorite: boolean; onFavorite: () => void; onDetails: () => void; onChat: () => void }) {
  return <motion.article whileTap={{ scale: .995 }} style={{ marginBottom: 14, overflow: "hidden", background: CARD, border: `1px solid ${LINE}`, borderRadius: 20, boxShadow: "0 12px 28px rgba(0,0,0,.18)" }}>
    ...
    <div style={{ padding: 14 }}><h2 style={{ margin: 0, fontSize: 16, fontWeight: 850 }}>{item.title}</h2><div style={{ display: "flex", alignItems: "center", gap: 5, marginTop: 7, color: DIM, fontSize: 11 }}><MapPin size={14} color={GOLD} /> {item.location}</div><div style={{ display: "flex", flexWrap: "wrap", gap: 9, marginTop: 13, paddingBottom: 12, borderBottom: `1px solid ${LINE}`, color: DIM, fontSize: 10 }}><span><BedDouble size={13} style={{ verticalAlign: "-3px", marginRight: 4 }} />{item.beds || "—"} Beds</span><span><Bath size={13} style={{ verticalAlign: "-3px", marginRight: 4 }} />{item.baths || "—"} Baths</span><span>▦ {item.area}</span></div><div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, marginTop: 12 }}><div style={{ display: "flex", alignItems: "center", gap: 5, minWidth: 0, color: DIM, fontSize: 10 }}><ShieldCheck size={14} color={GREEN} /><span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{item.seller}</span></div><span style={{ color: GREEN, fontSize: 9, fontWeight: 800 }}>Sovereign Verified Agency</span></div><div style={{ display: "flex", gap: 8, marginTop: 14 }}><Link href={`/market/properties/${item.id}`} onClick={(event) => { event.stopPropagation(); onDetails(); }} style={{ ...primaryButton, textDecoration: "none" }} aria-label={`View details for ${item.title}`}>View Details</Link><button onClick={onChat} style={secondaryButton}><MessageCircle size={15} /> Chat</button></div></div>
  </motion.article>;
}
