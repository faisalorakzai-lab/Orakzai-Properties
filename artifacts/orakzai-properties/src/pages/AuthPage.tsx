  ...
  const handleSubmit = async () => {
    ...
    try {
      if (mode === "signin") { await signInWithEmailAndPassword(auth, email, password); }
        ...
        // Eagerly generate & cache OkzByte UID + deposit addresses for new user
        getUserCryptoProfile(cred.user.uid);
      }
      setLocation("/");
    } catch (err: any) {
      const code = err?.code ?? "";
...
function GoogleButton() {
  ...
  const handleGoogle = async () => {
    ...
    try { const provider = new GoogleAuthProvider(); provider.addScope("email"); provider.addScope("profile"); const cred = await signInWithPopup(auth, provider); getUserCryptoProfile(cred.user.uid); setLocation("/"); }
