...
import {
  MapPin, Home, ArrowRight, ShieldCheck,
  MessageCircle, BanIcon, BarChart2,
} from "lucide-react";
...
interface PropertyCardProps {
  ...
  /** If set, shows a "Trade" button that switches to Exchange mode with this pair */
  linkedToken?: string;
}
...
export function formatPrice(price: number | undefined | null, category?: string): string {
  ...
  const base =
    price >= 10_000_000 ? `PKR ${(price / 10_000_000).toFixed(2)} Crore` :
    price >= 100_000    ? `PKR ${(price / 100_000).toFixed(0)} Lakh` :
...
export default function PropertyCard({
  id, title, price, city, area, category, type, images,
  isVerified, isAvailable = true, ownerName, whatsappNumber, ownerPhone,
  linkedToken,
}: PropertyCardProps) {
  const [, navigate]             = useLocation();
  const { setMode, setActivePair } = useMode();
  ...
  /** Switch to Exchange mode, set the pair, and navigate to /trade */
  function handleTradeToken(e: React.MouseEvent) {
    e.stopPropagation();
    e.preventDefault();
    if (!linkedToken) return;
    setActivePair(linkedToken);
    setMode("exchange");
    navigate("/trade");
  }
        ...
        {linkedToken && (
          <div className="absolute bottom-3 left-3">
            <span className="text-[9px] font-bold tracking-widest uppercase px-2 py-0.5 rounded-full"
              style={{ background:"rgba(201,168,76,0.18)", border:"1px solid rgba(201,168,76,0.45)", color:"#C9A84C" }}>
              🔗 {linkedToken.split("/")[0]}
            </span>
          </div>
        )}
      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-between pt-3 border-t border-white/5">
          ...
          <div className="flex items-center gap-1.5">
            {/* Trade Token button — only for tokenized properties */}
            {linkedToken && !rented && (
              <button
                onClick={handleTradeToken}
                data-testid={`button-trade-${id}`}
                className="flex items-center gap-1 h-8 px-2.5 rounded-xl text-xs font-bold transition-all"
                style={{ background:"rgba(201,168,76,0.12)", border:"1px solid rgba(201,168,76,0.4)", color:"#C9A84C" }}
              >
                <BarChart2 className="h-3 w-3" /> Trade
              </button>
            )}
