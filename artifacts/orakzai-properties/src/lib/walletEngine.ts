...
export type WalletTx = TradeTx | DepositTx | WithdrawalTx;
...
export function getWithdrawalTxns(): WithdrawalTx[] {
  return getTxns()
    .filter((tx): tx is WithdrawalTx => tx.type === "withdrawal")
    .sort((a, b) => b.time.localeCompare(a.time));
}

export function recordWithdrawal(
  withdrawal: Omit<WithdrawalTx, "id" | "type">,
): WithdrawalTx {
  const tx: WithdrawalTx = { ...withdrawal, id: uid(), type: "withdrawal" };
  saveTxns([tx, ...getTxns()]);
  return tx;
}
...
export function updateWithdrawalStatus(
  ...
  let updated: WithdrawalTx | null = null;
  const txns = getTxns().map(tx => {
    if (tx.type !== "withdrawal" || tx.id !== id) return tx;
    updated = { ...tx, status };
    return updated;
  });
  if (updated) saveTxns(txns);
  return updated;
}
